...
/*         if (o->specular != -1) // коэффициент s должен быть положительным числом, если объект обладает свойством зеркальности
    ...
    i = i > 1 ? 1 : i;
    result_color.red = (start_color->red * i > 255) ? 255 : (start_color->red * i);
    result_color.green = (start_color->green * i > 255) ? 255 : (start_color->green * i);
    result_color.blue = (start_color->blue * i > 255) ? 255 : (start_color->blue * i);
    return (result_color);
}
